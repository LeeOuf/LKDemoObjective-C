...
@interface LKPagingViewInfo : NSObject

@property (nonatomic, assign) CGPoint offset;

@end

@implementation LKPagingViewInfo

@end

@interface LKPagingView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) NSMutableArray<LKPagingViewInfo *> *scrollViewsInfo;     // 记录scrollViews当前滚动位置等信息
@property (nonatomic, assign) NSInteger lastPageIndex;
@property (nonatomic, assign) CGFloat extraTopHeight;

@end

@implementation LKPagingView
...
- (void)setupPageView
{
    self.pageView = [[LKPagingScrollView alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height)];
...
- (void)setupScrollViews
{
    ...
    for (; i < self.scrollViews.count; i++)
    {
        ...
        LKPagingViewInfo *info = [[LKPagingViewInfo alloc] init];
...
- (LKPagingViewInfo *)getScrollViewInfo:(UIScrollView *)scrollView
{
...
{
    LKPagingViewInfo *scrollViewInfo = [self getScrollViewInfo:scrollView];
...
- (void)resetAllInsets
{
    ...
    for (LKPagingViewInfo *info in self.scrollViewsInfo)
    {
        info.offset = CGPointMake(0, -insets.top);
    }
